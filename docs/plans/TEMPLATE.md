...
decision this milestone settles is amended into the spec by the task that implements it, in a
`docs:` commit before that task's `feat:` commit.
...
**Break-verify:** which assertion gets broken, and what failure the run must print. A task is
not done until every assertion it added has been broken and seen to fail, with the output
pasted into the commit body (constitution principle 3).

<!-- At most about 60 lines per task, this heading to the next. A task that will not fit is
     two tasks, or its design is unsettled and belongs in the spec first. -->
