...
- **Timing in the wrapper tests.** Heartbeat and poll intervals are 1 s in tests; assertions use ranges and `wait_for` with generous timeouts. A slow CI runner shows up as a `wait_for` timeout, never as a false pass.
