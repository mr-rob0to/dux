...
- Restart is a non-event. All state is files plus tmux; conversation memory is
  never authoritative.
...
~/Documents/dev/projects/dux/
  ...
  bin/
    ...
    dux-worker-wrap         runs inside the tmux window: claude -p + status protocol
...
```
- fitfights_api  path=~/Documents/dev/projects/fitfights_api  base=staging  worktree=make  (added 2026-09-03)
- fitfights_ios  path=~/Documents/dev/projects/fitfights_ios  base=main     worktree=git   (added 2026-09-03)
```
...
- the tmux server is not running or a window for the id exists;
- the lock is not held by this Dux session.

Otherwise: fetch, create worktree via the project's mechanism, copy the brief in,
open tmux window `dux-<id>`, run `dux-worker-wrap`, record window id and
`running` in `backlog.md`.
...
kills the window, marks `done` or `failed` in `backlog.md`. Task folder is kept.
...
30 seconds it reads the last line of every running task's `status.log` and the
tmux window list, and appends to `state/events.log` only when:

- the last state changed to `done`, `failed`, `blocked`, or `needs-decision`;
- no new line for 20 minutes while the window is alive (`stale: <id>`);
- the window is gone without a terminal state (`dead: <id>`).
...
On start, CLAUDE.md instructs Dux to run `dux-lock acquire`, then `dux-status`,
then arm the Monitor. If the lock is held by a live pid, Dux announces it is
read-only and skips spawn, teardown, and recover. On stop, the lock releases and
the watcher dies; workers keep running under tmux and are reconciled next start.

## 8. Fleet digest (`dux-status`)

Files and tmux only, no network. Per project, four lines: running, awaiting
you (needs-decision, blocked), ready to merge (done with PR), failed. Zero-count
lines omitted. `--prs` adds `gh pr view` state per ready PR.
...
9. PR body. Step 8 fills the repo's `.github/PULL_REQUEST_TEMPLATE.md` (section
   10). Verbose material goes inside `<details>`.
...
| Dux restarted mid-task | reconcile from files and tmux; workers unaffected |
...
1. Skeleton: repo, CLAUDE.md, registry, `dux-doctor`, `dux-lock`, `dux-project`,
   PR template, tests harness with fake claude.
2. Dispatch: `dux-brief`, `dux-worktree`, `dux-spawn`, `dux-worker-wrap`,
   `dux-teardown`, `dux-dispatch` skill, end-to-end happy path.
3. Supervision: `dux-watch`, Monitor arming, `dux-status`, `dux-notify`,
   `dux-recover`, stale and dead paths.
4. `/ship` port: the nine changes in section 9, break-verified.
5. Dogfood: register fitfights_api and fitfights_ios, run one scout and one ship
   task each, fix what breaks.
...
- Backlog in local markdown, not GitHub Issues.
