...
{{feedback}}- Run the ship gate again. It updates pull request {{PR}}; never open another.
...
{{approval}}- Run the ship gate, then append `done: PR <url>` to the file named by `$DUX_STATUS_LOG` and wait at the prompt. This round gets one terminal line, and every other rule in the brief still holds.
