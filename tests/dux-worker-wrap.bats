...
@test "a pane that never holds the harness is a refusal, not a silent supervision" {
  ...
  # that can leave a session running with no group file, unsupervised.
  [[ "$output" == *"the harness for $id did not appear in its pane within 2s; if a session is running there, end it in the tab before starting $id again"* ]]
  [ "$(handoff_status)" = "failed: wrapper: the harness for $id did not appear in its pane within 2s; if a session is running there, end it in the tab before starting $id again" ]
  [ "$(handoff_event)" = failed ]
