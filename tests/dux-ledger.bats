...
@test "unack lets the same state wake again" {
  dux-ledger add t1 proj scout local
  dux-ledger set t1 state stale; dux-ledger ack t1 stale
...
@test "ack refuses when the task moved past the handled state" {
  dux-ledger add t1 proj scout local
  dux-ledger set t1 state stale
  dux-ledger set t1 state done
  run dux-ledger ack t1 stale
  [ "$status" -eq 2 ]
  [[ "$output" == "finding: task t1 is now done; refusing to acknowledge handled state stale"* ]]
  [ "$(dux-ledger get t1 acked)" = - ]
...
@test "ack refuses unknown and bad ids, and set cannot write acked" {
  run dux-ledger ack nope done; [ "$status" -eq 2 ]; [[ "$output" == "finding: task nope not in ledger"* ]]
  run dux-ledger ack ../x done; [ "$status" -eq 2 ]; [[ "$output" == "finding: task id must match"* ]]
  dux-ledger add t1 proj scout local
